...
export function ChatSidebar({ className }: ChatSidebarProps) {
  ...
  const HEADER_HEIGHT = 52 // px - header with AI Assistant title
  const INPUT_HEIGHT = 140 // px - approximate input area height
  const PAGE_HEADER = 52 // px - top page header
  
  return (
    <div 
      className={`flex flex-col bg-zinc-900/80 ${className || ''}`}
      style={{ height: `calc(100vh - ${PAGE_HEADER}px)` }}
    >
      {/* Header - fixed height */}
      <div 
        className="px-4 py-3 border-b border-zinc-800 bg-zinc-900"
      ...
      <div 
        className="overflow-y-auto bg-zinc-800/20"
              ...
              {chatMessages.map((msg) => (
                ...
                >
                  ...
                  <div className={`flex-1 min-w-0 ${msg.role === 'user' ? 'text-right' : ''}`}>
                    {msg.selectedText && (
                      <div className="text-xs text-zinc-500 mb-1 italic truncate">
      ...
      {/* Error */}
      {aiError && (
        <div className="px-4 py-2 bg-rose-500/10 text-rose-400 text-sm border-t border-zinc-800">
          {aiError}
        </div>
      )}
      
      {/* Input Area - fixed at bottom */}
      <div className="border-t border-zinc-800 bg-zinc-900" style={{ flexShrink: 0 }}>
